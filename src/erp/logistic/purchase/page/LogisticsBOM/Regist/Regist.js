...
const Regist = (props) => {
    ...
    const batchBom =  useCallback(() => {  //새로운 항목 추가 후 일괄저장 버튼 클릭 시 batchBom 호출함 (resultList에 추가한 row 내용 담겨 있음)
    ...
    const batchAll = () => {
        ...
        positionGridApi.forEachNode((node, index) => {  //forEacthNode -> forEachNode
        ...
        if(resultList.length !== 0 && confirmStatus) {
            batchBom(); //새로운 항목 추가 후 일괄저장 버튼 클릭 시 batchBom 호출함
